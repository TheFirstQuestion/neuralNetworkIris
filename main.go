...
type Neuron struct {
	...
	outgoWeights []float64
	// Change in outgoing weight values
	outgoDeltas []float64
}
...
// How many neurons we want
// We have four inputs
const numInputNeuron = 4
// Changing this should change accuracy?
const numHiddenNeuron = 4
...
func main() {
	...
	// Create a Network using our Network struct
...
func (net *Network) initNetwork(numInputs, numHidden, numOutput int) {
	...
	for i := range net.inputLayer {
		// Allocate memory for outgoing weights
...
func (net *Network) Train(trainData [][]float64, maxEpochs int, maxError, learnRate, momentum float64) int {
	...
	for epoch = 0; epoch < maxEpochs; epoch++ {
		...
		for _, i := range indices {
			// inputData to hold the measurements
			// targetData to hold the classification
...
func (net *Network) isCorrect(targetData []float64) bool {
	// If our output possibilities don't match the real outputs, that is bad
	...
	// Keep track of the highest value we're outputting
	var maxIndex int
	for i := range net.outputLayer {
	...
	// Return true if the highest value here is a 1 *WHY*
...
func (net *Network) backProp(targetData []float64, learnRate, momentum float64) {
	...
	// Why do we calculate it like this??????
...
func shuffleIndices(indices []int) []int {
	...
	// Produces and unbiased, equally likely permutations
...
func prepData(data [][]float64) (trainData, testData [][]float64) {
	// 80% of data will be training data
